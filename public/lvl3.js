...
// UPDATE VIGNETTE INTENSITY BASED ON DEATH TIMEOUT
...
export function resetLevel() {
    ...
    setupLevel(); // Reapply the level configurations
...
function setupLevel() {
    ...
    // ADD FLASHLIGHT TARGET TO SCENE
    scene.add(flashLightTarget);
    flashLight.target = flashLightTarget;

    // ADD FLASHLIGHT TO PLAYER CHARACTER
    cube.add(flashHolder); // Attach it to the cube

    // SET BACKGROUND COLOUR
    scene.background = new THREE.Color( 0x000000 );

    // LOAD THE MESH FOR THE MAP
    ...
    // LOAD THE MESH FOR THE CHESTS
    gltfLoader.load('./assets/inferno/cgv-inferno-map-chests-mesh.glb', (gltf) => {
        // Add the loaded infernoChests to the scene
        infernoChests = gltf.scene;
        // Position the infernoChests to the right of the plane
    ...
    // LOAD THE MESH FOR THE (INVISIBLE) WALLS
...
// UPDATE BOUNDING BOXES IN RENDER LOOP
...
window.addEventListener('mousemove', function(event) {
    ...
    if (infernoMap) {  // Check if infernoMap is defined
        let intersects = raycaster.intersectObject(infernoMap);
...
// STOP MOVING WHEN KEY STOPS BEING PRESSED
...
export function render() {
    ...
    // FLASHLIGHT BATTERY TIMEOUT
    if (flashTimeout > 100) {
        darknessTimeout = 100;
        // FLASHLIGHT FLICKER RANDOMISER
    ...
    // SHOW DEATH SCREEN AFTER PLAYER IN DARK FOR TOO LONG
    ...
    // TIMER TO RESET LEVEL AFTER DEATH
    if(resetLevelTimeout <= 0){
        resetLevelTimeout = 10;
        resetLevel();
    }

    // VIGNETTE TO WARN PLAYER OF BEING IN DARKNESS
